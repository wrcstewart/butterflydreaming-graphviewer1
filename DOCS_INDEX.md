...
| **AV/README.md** | 09-14 | **How to build an Ancillary Viewer**, written for a third party. What an AV is (presentation, not editing), the MST token, the shim, the two traps that cost real time (named windows survive navigation; an iframe with a `src` in HTML loses BD_READY), and the measured ~30 ms socket latency. |
| **module_data_modes.md** | 09-14 | **The LD / SD / UD vocabulary**, fixed so it stops being re-explained: Live Data (token → socket → live DB, read+write), Static Data (published snapshot), URL Data (carried in the link). Plus MST, MDP, NR, OH. Argues NR — one payload shape across all three — is the integration and the rest is transport. |
